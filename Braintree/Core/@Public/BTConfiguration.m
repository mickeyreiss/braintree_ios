...
@property (nonatomic, readwrite, strong) BTJSON *configurationParser;

@end

@implementation BTConfiguration

- (instancetype)initWithResponseParser:(BTJSON *)responseParser error:(NSError **)error {
    self = [super init];
    if (self) {
...
- (BTJSON *)decodeConfiguration:(NSString *)rawConfigurationString error:(NSError * __autoreleasing *)error {
    NSError *JSONError;
    ...
    return [BTJSON JSONWithDictionary:rawConfiguration];
}
...
- (BTConfigurationVenmoStatus)venmoStatus {
    NSString *status = self.configurationParser[BTConfigurationKeyVenmo].asString;
    if ([status isEqualToString:BTVenmoStatusValueProduction]) {
        return BTConfigurationVenmoStatusProduction;
    } else if([status isEqualToString:BTVenmoStatusValueOffline]) {
        return BTConfigurationVenmoStatusOffline;
    } else {
        return BTConfigurationVenmoStatusOff;
    }
}
