...
#pragma clang diagnostic pop
    ...
    describe(@"clients with PayPal activated", ^{
        ...
        describe(@"createPayPalPaymentResourceWithAmount:", ^{
            pending(@"a reasonable way to set up this test", ^{
                it(@"creates a single payments resource for use in checkout", ^{
                    XCTestExpectation *expectation = [self expectationWithDescription:@""];
                    NSString *redirectUrl = @"https://example.com/redirect";
                    NSString *cancelUrl = @"https://example.com/cancel";
                    [testClient createPayPalPaymentResourceWithAmount:[NSDecimalNumber decimalNumberWithString:@"1"]
                                                          redirectUri:redirectUrl
                                                            cancelUri:cancelUrl
                                             applicationCorrelationId:@"some metadata id"
                                                              success:^(id response){
                                                                  NSLog(@"%@", response);
                                                                  [expectation fulfill];
                                                              } failaure:^(NSError *error) {
                                                                  NSLog(@"%@", error);
                                                              }];
                    [self waitForExpectationsWithTimeout:10 handler:nil];
                });
