...
* Features
  ...
  * `BTDataCollector` now includes PayPal application correlation ID in device data blob
...
* Bug fixes
  ...
  * Fix iOS 8 `CoreLocation` deprecation in `BTDataCollector`
...
* Update BTDataCollector (fraud) API to match Braintree-Data.js
...
* Integration Change:
  * `Braintree/data` is no longer a default subspec. If you are currently using `BTDataCollector`, please add `pod "Braintree/data"` to your `Podfile`.
...
* Features:
  * :new: `BTDataCollector` - Advanced fraud solution based on Kount SDK
